  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-5">
        ...
        {reviews.map((r) => (
          <div key={r.id} className="border-b border-ink/10 dark:border-white/10 pb-5">
            <div className="mb-1 flex items-center gap-2.5">
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 dark:bg-white/10 text-xs font-bold text-brand-700 dark:text-gold-300">
...
export function ReviewForm({ bookableType, bookableId, onSubmitted }) {
  ...
  return (
    <div className="mt-6 rounded-[20px] border border-ink/10 dark:border-white/10 p-5">
      <h4 className="mb-3 text-sm font-semibold text-ink dark:text-white">{t('detail.writeReview')}</h4>
