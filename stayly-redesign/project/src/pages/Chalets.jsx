  ...
  return (
    <div className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 sm:py-8">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-3xl font-medium text-ink dark:text-white sm:text-4xl">{t('nav.chalets')}</h1>
        <div className="flex items-center gap-2">
          <input
            placeholder="Search by name…"
            value={search}
            onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            className="w-40 rounded-full border border-ink/15 dark:border-white/15 px-4 py-2 text-sm outline-none focus:border-brand-400 sm:w-56"
          />
          <Button variant="outline" size="md" onClick={() => setFiltersOpen(true)}>
            <AdjustmentsHorizontalIcon className="h-4 w-4" /> {t('listing.filters')}
          </Button>
          <div className="flex rounded-full border border-ink/15 dark:border-white/15 p-1 lg:hidden">
            <button
              onClick={() => setMobileView('list')}
              className={`rounded-full p-1.5 ${mobileView === 'list' ? 'bg-brand-800 text-canvas' : 'text-ink/50 dark:text-white/50'}`}
            >
              <ListBulletIcon className="h-4 w-4" />
            </button>
            <button
              onClick={() => setMobileView('map')}
              className={`rounded-full p-1.5 ${mobileView === 'map' ? 'bg-brand-800 text-canvas' : 'text-ink/50 dark:text-white/50'}`}
            >
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_1fr] lg:h-[calc(100vh-180px)]">
        <div className={`${mobileView === 'map' ? 'hidden' : 'block'} overflow-y-auto pr-1 lg:block`}>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-[20px] border border-dashed border-ink/20 dark:border-white/20 py-16 text-center">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {chalets.map((c) => (
                  <div
                    ...
                    className={`rounded-[20px] transition ${hoveredId === c.id ? 'ring-2 ring-gold-400' : ''}`}
                  >
                ...
                <div className="mt-6 flex items-center justify-center gap-1.5">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <button
                      key={p}
                      onClick={() => setPage(p)}
                      className={`h-9 w-9 rounded-full text-sm font-semibold transition ${p === page ? 'bg-brand-800 text-canvas' : 'text-ink/60 dark:text-white/60 hover:bg-ink/5 dark:hover:bg-white/10'}`}
                    >
                      {p}
                    </button>
                  ))}
