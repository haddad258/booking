...
export default function BookingWizard() {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10 sm:py-16">
      <div className="rounded-[24px] border border-ink/10 dark:border-white/10 bg-white dark:bg-brand-800 p-6 sm:p-8">
        <div className="mb-8 flex items-center justify-between">
          {STEPS.map((s, i) => (
            <div key={s} className="flex flex-1 items-center">
              <div className={[
                'flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold',
                i <= activeStep ? 'bg-brand-800 text-canvas' : 'bg-ink/5 dark:bg-white/5 text-ink/40 dark:text-white/40',
              ].join(' ')}>
                {i + 1}
              </div>
              ...
              {i < STEPS.length - 1 && <div className={`mx-2 h-0.5 flex-1 ${i < activeStep ? 'bg-brand-800' : 'bg-ink/10 dark:bg-white/5'}`} />}
            </div>
          ))}
        </div>
        ...
        {activeStep === 0 && (
          <div>
            ...
            {nights > 0 && (
              <div className="mt-4 rounded-2xl bg-brand-50 dark:bg-white/5 p-4">
                <p className="text-sm text-ink/70 dark:text-white/70">${pricePerNight} × {nights} nights</p>
                <p className="font-display text-xl font-medium text-ink dark:text-white">{t('booking.total')}: ${total.toFixed(2)}</p>
              </div>
            )}
        ...
        {!user && activeStep === 1 && (
          <div>
            ...
            <label className="mt-4 flex items-start gap-2.5 rounded-2xl border border-ink/10 p-3.5 dark:border-white/10">
              <input
                type="checkbox"
                checked={createAccount}
                onChange={(e) => setCreateAccount(e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded border-ink/30 text-brand-600 focus:ring-brand-300"
              />
              <span>
        ...
        {activeStep === reviewStepIndex && (
          <div>
            ...
            <div className="mb-4 border-t border-ink/10 dark:border-white/10 pt-4 text-sm text-ink/80 dark:text-white/80 space-y-1.5">
              {!user && <p>{firstName} {lastName} · {email}</p>}
              <p>Check-in: {checkIn}</p>
              <p>Check-out: {checkOut}</p>
              <p>Guests: {adults} adults, {children} children</p>
              <p className="font-display pt-1 text-lg font-semibold text-ink dark:text-white">{t('booking.total')}: ${total.toFixed(2)}</p>
            </div>
