...
export default function Register() {
  ...
  if (createdUsername) {
    return (
      <div className="text-center">
        <h1 className="font-display mb-4 text-2xl font-medium text-ink dark:text-white">{t('nav.register')} 🎉</h1>
  ...
  return (
    <div>
      <h1 className="font-display mb-6 text-2xl font-medium text-ink dark:text-white">{t('auth.register')}</h1>
      ...
      <p className="mt-6 text-center text-sm text-ink/60 dark:text-white/60">
        {t('auth.haveAccount')} <RouterLink to="/login" className="font-semibold text-brand-700 dark:text-gold-300 hover:underline">{t('nav.login')}</RouterLink>
      </p>
    </div>
  );
}
