...
export default function Login() {
  ...
  return (
    <div>
      <h1 className="font-display mb-6 text-2xl font-medium text-ink dark:text-white">{t('auth.login')}</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="text-right">
          <RouterLink to="/forgot-password" className="text-sm font-semibold text-brand-700 dark:text-gold-300 hover:underline">{t('auth.forgotPassword')}</RouterLink>
        </div>
      ...
      <p className="mt-6 text-center text-sm text-ink/60 dark:text-white/60">
        {t('auth.noAccount')} <RouterLink to="/register" className="font-semibold text-brand-700 dark:text-gold-300 hover:underline">{t('nav.register')}</RouterLink>
      </p>
    </div>
  );
}
