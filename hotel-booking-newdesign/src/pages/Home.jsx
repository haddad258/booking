    ...
    <Box>
      ...
      <Box
        sx={{
          ...
          display: 'flex',
          alignItems: 'center',
          backgroundImage: `linear-gradient(160deg, rgba(15,23,42,0.55), rgba(79,70,229,0.45)), url(${HERO_IMAGE})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
        }}
      >
        <Container maxWidth="lg" sx={{ textAlign: 'center', color: '#fff', px: { xs: 2, sm: 3 } }}>
          <Typography
            variant="h1"
            sx={{ mb: 2, fontSize: { xs: '2.4rem', sm: '3rem', md: '4rem' }, lineHeight: 1.05 }}
          >
            {t('home.heroTitle')}
          </Typography>
          <Typography variant="h6" sx={{ mb: 5, opacity: 0.92, fontWeight: 500, maxWidth: 560, mx: 'auto' }}>
            {t('home.heroSubtitle')}
          </Typography>
          <SearchBar />
        </Container>
              ...
              <Grid item xs={6} md={3} key={d.name}>
                <Box
                  ...
                  sx={{
                    height: 200,
                    borderRadius: 4,
                    backgroundImage: `linear-gradient(180deg, transparent 40%, rgba(0,0,0,0.6)), url(${d.image})`,
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                    display: 'flex',
                    alignItems: 'flex-end',
      ...
      <Box sx={{ bgcolor: 'primary.main', color: '#fff', py: 8 }}>
        <Container maxWidth="lg">
          <Typography variant="h4" sx={{ mb: 4, textAlign: 'center' }}>{t('home.testimonials')}</Typography>
          <Grid container spacing={3}>
            {TESTIMONIALS.map((tst) => (
              <Grid item xs={12} md={4} key={tst.name}>
                <Box sx={{ p: 3, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', height: '100%' }}>
                  ...
                  <Stack direction="row" alignItems="center" spacing={1.5}>
                    <Avatar sx={{ width: 32, height: 32, backgroundImage: 'linear-gradient(135deg, #4F46E5 0%, #06B6D4 100%)' }}>{tst.name[0]}</Avatar>
