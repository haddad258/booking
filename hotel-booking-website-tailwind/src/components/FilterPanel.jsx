...
export default function FilterPanel({ filters, onChange, amenities = [], showRating }) {
  ...
  return (
    <div className="rounded-2xl border border-brand-800/10 bg-white p-5">
      <h3 className="mb-4 font-display text-lg font-semibold text-ink">{t('listing.filters')}</h3>

      <div className="mb-5">
        <p className="mb-2 text-sm font-semibold text-ink">{t('listing.priceRange')}</p>
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-full rounded-lg border border-brand-800/15 px-2.5 py-1.5 text-sm outline-none focus:border-gold-400"
          />
      ...
      {showRating && (
        <div className="mb-5 border-t border-brand-800/10 pt-4">
          <p className="mb-2 text-sm font-semibold text-ink">{t('listing.rating')}</p>
            ...
            {[5, 4, 3, 2].map((r) => (
              <label key={r} className="flex items-center gap-2 text-sm text-ink/80">
                <input
                  ...
                  className="h-4 w-4 rounded border-brand-800/30 text-brand-700 focus:ring-gold-300"
                />
                {r}+ stars
              </label>
            ))}
      ...
      {amenities.length > 0 && (
        <div className="mb-5 max-h-56 overflow-y-auto border-t border-brand-800/10 pt-4">
          <p className="mb-2 text-sm font-semibold text-ink">{t('listing.amenities')}</p>
          <div className="flex flex-col gap-1.5">
            {amenities.map((a) => (
