...
export default function BookingWizard() {
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10 sm:py-16">
      <div className="rounded-2xl border border-brand-800/10 dark:border-white/10 bg-white dark:bg-brand-800 p-6 sm:p-8">
        <div className="mb-8 flex items-center justify-between">
          {STEPS.map((s, i) => (
            <div key={s} className="flex flex-1 items-center">
              <div className={[
                'flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold',
                i <= activeStep ? 'bg-brand-800 text-white' : 'bg-brand-50 dark:bg-white/5 text-ink/40 dark:text-white/40',
              ].join(' ')}>
                {i + 1}
              </div>
              ...
              {i < STEPS.length - 1 && <div className={`mx-2 h-0.5 flex-1 ${i < activeStep ? 'bg-brand-800' : 'bg-brand-50 dark:bg-white/5'}`} />}
            </div>
          ))}
        </div>
        ...
        {!user && activeStep === 1 && (
          <div>
            ...
            <label className="mt-4 flex items-start gap-2.5 rounded-xl border border-brand-800/10 p-3.5 dark:border-white/10">
              <input
                type="checkbox"
                checked={createAccount}
                onChange={(e) => setCreateAccount(e.target.checked)}
                className="mt-0.5 h-4 w-4 rounded border-brand-800/30 text-gold-500 focus:ring-gold-400"
              />
              <span>
        ...
        {activeStep === reviewStepIndex && (
          <div>
            ...
            <div className="mb-4 border-t border-brand-800/10 dark:border-white/10 pt-4 text-sm text-ink/80 dark:text-white/80 space-y-1.5">
