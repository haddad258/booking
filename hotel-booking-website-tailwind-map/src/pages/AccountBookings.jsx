...
export default function AccountBookings() {
  ...
  return (
    <div className="rounded-2xl border border-brand-800/10 dark:border-white/10 bg-white dark:bg-brand-800 p-6">
      <h2 className="font-display mb-4 text-lg font-semibold text-ink dark:text-white">Your bookings</h2>
      {error && <p className="mb-3 text-sm text-red-600">{error}</p>}

      {loading ? (
        <Spinner className="h-6 w-6" />
      ) : bookings.length === 0 ? (
        <p className="text-sm text-ink/50 dark:text-white/50">No bookings yet.</p>
      ) : (
        <div className="flex flex-col gap-3">
          {bookings.map((b) => (
            <div key={b.id} className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-brand-800/10 dark:border-white/10 p-4">
              <div>
