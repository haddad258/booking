...
export default function FilterPanel({ filters, onChange, amenities = [], showRating }) {
  ...
  return (
    <div className="rounded-3xl border border-brand-500/10 dark:border-white/10 bg-white dark:bg-brand-900 p-5">
      <h3 className="eyebrow mb-5 text-ink dark:text-white">{t('listing.filters')}</h3>
      ...
      <div className="mb-5">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-full rounded-xl border border-brand-500/15 dark:border-white/15 px-2.5 py-1.5 text-sm outline-none focus:border-brand-400"
          />
      ...
      {showRating && (
        <div className="mb-5 border-t border-brand-500/10 dark:border-white/10 pt-4">
            ...
            {[5, 4, 3, 2].map((r) => (
              <label key={r} className="flex items-center gap-2 text-sm text-ink/80 dark:text-white/80">
                <input
                  ...
                  className="h-4 w-4 rounded border-brand-500/30 dark:border-white/30 text-brand-600 dark:text-gold-300 focus:ring-brand-400"
                />
                {r}+ stars
              </label>
            ))}
      ...
      {amenities.length > 0 && (
        <div className="mb-5 max-h-56 overflow-y-auto border-t border-brand-500/10 dark:border-white/10 pt-4">
