...
export default function PropertyLocationMap({ name, address, city, country, latitude, longitude }) {
  ...
  if (!hasCoords) {
    return (
      <div className="flex h-64 items-center justify-center rounded-3xl border border-dashed border-brand-500/25 dark:border-white/20 bg-brand-50 dark:bg-white/5 text-center sm:h-80">
        <div className="px-6">
  ...
  return (
    <div className="h-64 overflow-hidden rounded-3xl border border-brand-500/10 dark:border-white/10 sm:h-80">
      <MapContainer
        center={position}
        zoom={14}
