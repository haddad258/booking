  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="font-display text-3xl font-extrabold text-ink dark:text-white sm:text-4xl">{hotel.name}</h1>
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <h2 className="eyebrow mb-4 text-ink dark:text-white">{t('detail.amenities')}</h2>
          <div className="mb-8 flex flex-wrap gap-2">
            {hotel.amenities?.map((a) => <Badge key={a.id}>{a.name}</Badge>)}
          </div>

          <div className="mb-8 border-t border-brand-500/10 dark:border-white/10" />

          <h2 className="eyebrow mb-4 text-ink dark:text-white">{t('detail.location')}</h2>
          <div className="mb-8">
            <PropertyLocationMap
              name={hotel.name}
          ...
          <h2 className="eyebrow mb-4 text-ink dark:text-white">{t('detail.reviews')}</h2>
        ...
        <div>
          <div className="sticky top-24 rounded-3xl border border-brand-500/10 dark:border-white/10 bg-white dark:bg-brand-900 p-6 shadow-xl shadow-brand-500/5">
            <h2 className="eyebrow mb-5 text-ink dark:text-white">{t('detail.selectRoom')}</h2>
            <div className="flex flex-col gap-3">
              {hotel.rooms?.length ? (
                hotel.rooms.map((room) => (
                  <div key={room.id} className="rounded-2xl border border-brand-500/10 dark:border-white/10 p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="font-display font-extrabold text-gradient">${Number(room.price).toFixed(0)}</p>
                    </div>
                    <Button
                      fullWidth
                      size="sm"
      ...
      {similar.length > 0 && (
        <div className="mt-14">
          <h2 className="font-display mb-6 text-2xl font-extrabold text-ink dark:text-white">{t('detail.similar')}</h2>
